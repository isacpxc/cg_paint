...
void desenharCena(CenaGrafica *cena) {
  ...
  for (int i = 0; i < cena->qtd_poligonos; i++) {
    ...
    if (p->preenchido) {
      ...
      glColor3f(p->cor_preenchimento[0], p->cor_preenchimento[1],
                p->cor_preenchimento[2]);
    }
...
int clicouNoPonto(PontoDesenho *ponto, Ponto clique) {
  if (clique.x <= ponto->posicao.x + TOLERANCIA &&
      clique.x >= ponto->posicao.x - TOLERANCIA) {
    if (clique.y <= ponto->posicao.y + TOLERANCIA &&
        clique.y >= ponto->posicao.y - TOLERANCIA) {
      return 1;
    }
  }
  return 0;
...
int clicouNoPoligono(Poligono *poligono, Ponto clique) {
  for (int i = 0; i < poligono->qtd_vertices; i++) {
    ...
    if (cohenSutherlandIntercepta(poligono->vertices[i], poligono->vertices[j],
                                  clique, TOLERANCIA)) {
      return 1;
    }
  }
