...
void atualizarPassoAnimacao(ContextoPaint *ctx) {
  ...
  if (modo_animacao == ANIM_CIRCULAR) {
    ...
    if (modo_animacao == ANIM_DIREITA)
      dx = vel;
    if (modo_animacao == ANIM_ESQUERDA)
      dx = -vel;
    if (modo_animacao == ANIM_CIMA)
      dy = vel;
    if (modo_animacao == ANIM_BAIXO)
      dy = -vel;
  }

  for (int i = 0; i < ctx->cena.qtd_pontos; i++) {
    if (!ctx->cena.pontos[i].selecionado)
      continue;
    ctx->cena.pontos[i].posicao.x += dx;
    ctx->cena.pontos[i].posicao.y += dy;
    if (ctx->cena.pontos[i].posicao.x > 800.0f)
      ctx->cena.pontos[i].posicao.x = 0.0f;
    if (ctx->cena.pontos[i].posicao.x < 0.0f)
      ctx->cena.pontos[i].posicao.x = 800.0f;
    if (ctx->cena.pontos[i].posicao.y > 600.0f)
      ctx->cena.pontos[i].posicao.y = 0.0f;
    if (ctx->cena.pontos[i].posicao.y < 0.0f)
      ctx->cena.pontos[i].posicao.y = 600.0f;
  }

  for (int i = 0; i < ctx->cena.qtd_retas; i++) {
    if (!ctx->cena.retas[i].selecionado)
      continue;
    ...
    if (ctx->cena.retas[i].p1.x > 800.0f) {
      ...
      ctx->cena.retas[i].p1.x = 0.0f;
      ctx->cena.retas[i].p2.x = tam;
    }
    if (ctx->cena.retas[i].p1.x < 0.0f) {
      float tam = ctx->cena.retas[i].p2.x - ctx->cena.retas[i].p1.x;
      ctx->cena.retas[i].p1.x = 800.0f;
      ctx->cena.retas[i].p2.x = 800.0f + tam;
    }
    if (ctx->cena.retas[i].p1.y > 600.0f) {
      float tam = ctx->cena.retas[i].p2.y - ctx->cena.retas[i].p1.y;
      ctx->cena.retas[i].p1.y = 0.0f;
      ctx->cena.retas[i].p2.y = tam;
    }
    if (ctx->cena.retas[i].p1.y < 0.0f) {
      float tam = ctx->cena.retas[i].p2.y - ctx->cena.retas[i].p1.y;
      ctx->cena.retas[i].p1.y = 600.0f;
      ctx->cena.retas[i].p2.y = 600.0f + tam;
    }
  }

  for (int i = 0; i < ctx->cena.qtd_poligonos; i++) {
    if (ctx->cena.poligonos[i].qtd_vertices == 0)
      continue;
    if (!ctx->cena.poligonos[i].selecionado)
      continue;
    ...
    if (refX > 800.0f || refX < 0.0f || refY > 600.0f || refY < 0.0f) {
      for (int j = 0; j < ctx->cena.poligonos[i].qtd_vertices; j++) {
        if (refX > 800.0f)
          ctx->cena.poligonos[i].vertices[j].x -= 800.0f;
        if (refX < 0.0f)
          ctx->cena.poligonos[i].vertices[j].x += 800.0f;
        if (refY > 600.0f)
          ctx->cena.poligonos[i].vertices[j].y -= 600.0f;
        if (refY < 0.0f)
          ctx->cena.poligonos[i].vertices[j].y += 600.0f;
      }
    }
